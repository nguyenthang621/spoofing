...
public class AtiClientService implements MAPServiceMobilityListener {
	...
	@Autowired
	private MAPConfiguration mapMan;

	@Autowired
	private AtiClientConfiguration props;

	@Autowired
	private SCCPConfiguration sccp;

	@Autowired
	MapDialogHandle dialogHandler;
	...
	public long performAtiRequest(String address, AtiClientConfiguration override) throws MAPException {
		...
		return doAtiRequest(address, override != null ? override : props);
	}
	...
	private long doAtiRequest(String address, AtiClientConfiguration override) throws MAPException {
		...
		GlobalTitle gtDest;
		if (props.isSubscriberIdentityTypeIsImsi()) {
			...
			gtDest = this.sccp.getParameterFactory().createGlobalTitle(imsi.getData(), 0,
					org.restcomm.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, ec,
					NatureOfAddress.INTERNATIONAL);
		} else {
			ISDNAddressString msisdn = mapProvider.getMAPParameterFactory().createISDNAddressString(props.getAddressNature(), props.getNumberingPlan(), address);
			subscriberIdentity = mapProvider.getMAPParameterFactory().createSubscriberIdentity(msisdn);
			gtDest = this.sccp.getParameterFactory().createGlobalTitle(msisdn.getAddress(), 0,
					org.restcomm.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, ec,
					NatureOfAddress.INTERNATIONAL);
		}
		RequestedInfo requestedInfo = mapProvider.getMAPParameterFactory().createRequestedInfo(
				override.isGetLocationInformation(), override.isGetSubscriberState(), null,
				override.isGetCurrentLocation(), override.getGetRequestedDomain(), override.isGetImei(),
				override.isGetMsClassmark(), override.isGetMnpRequestedInfo());
		ISDNAddressString gsmSCFAddress = mapProvider.getMAPParameterFactory()
				.createISDNAddressString(props.getAddressNature(), props.getNumberingPlan(), props.getGsmScfAddress());

		GlobalTitle gtSrc = this.sccp.getParameterFactory().createGlobalTitle(props.getSrcGt(), 0,
				org.restcomm.protocols.ss7.indicator.NumberingPlan.ISDN_TELEPHONY, ec, NatureOfAddress.INTERNATIONAL);
		SccpAddress origin = this.sccp.getParameterFactory()
				.createSccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, gtSrc, 0, 8);
		SccpAddress dest = this.sccp.getParameterFactory()
				.createSccpAddress(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, gtDest, 0, 6);

		MAPDialogMobility curDialog = mapProvider.getMAPServiceMobility().createNewDialog(mapAppContext, origin, null,
				dest, null);
		...
		Metrics.globalRegistry.counter("map_request", "type", "AnyTimeInterrogation", "exception", "none").increment();
	...
	public void onAnyTimeInterrogationResponse(AnyTimeInterrogationResponse ind) {
		...
		try {
			...
			if (ind.getSubscriberInfo() != null) {
				...
				if (subInfo.getIMEI() != null)
					meta.setImei(subInfo.getIMEI().getIMEI());
				...
				if (subInfo.getLocationInformation() != null) {
					...
					if ((subInfo.getLocationInformation().getCellGlobalIdOrServiceAreaIdOrLAI() != null)
							&& (subInfo.getLocationInformation().getCellGlobalIdOrServiceAreaIdOrLAI()
									.getCellGlobalIdOrServiceAreaIdFixedLength() != null)) {
						try {
							meta.setMcc(subInfo.getLocationInformation().getCellGlobalIdOrServiceAreaIdOrLAI()
									.getCellGlobalIdOrServiceAreaIdFixedLength().getMCC());
							meta.setMnc(subInfo.getLocationInformation().getCellGlobalIdOrServiceAreaIdOrLAI()
			...
			serializableRedisTemplate.opsForValue().set(ATI_CACHE + invokeId, meta);
			redisson.getCountDownLatch(Constants.TXN + invokeId).countDown();
			Metrics.globalRegistry.counter("map_response", "type", "AnyTimeInterrogation", "exception", "none")
					.increment();
	...
	public void onRejectComponent(MAPDialog mapDialog, Long invokeId, Problem problem, boolean isLocalOriginated) {
		log.debug("onRejectComponent: mapDialog {} invokeId {} problem {} isLocalOriginated {}", mapDialog, invokeId,
				problem, isLocalOriginated);
		if (isLocalOriginated)
			MapDialogHandle.needSendClose = true;
		dialogHandler.handleTxn(mapDialog.getLocalDialogId(), 5);
		Metrics.globalRegistry.counter("map_response", "type", "AnyTimeInterrogation", "exception", "REJECT")
				.increment();
	}

	@Override
	public void onErrorComponent(MAPDialog mapDialog, Long invokeId, MAPErrorMessage mapErrorMessage) {
		log.debug("onErrorComponent: mapDialog {} invokeId {} mapErrorMessage {}", mapDialog, invokeId,
				mapErrorMessage);
		// Extract data from index
		...
		dialogHandler.handleTxn(mapDialog.getLocalDialogId(), 5);
		Metrics.globalRegistry.counter("map_response", "type", "AnyTimeInterrogation", "exception", error).increment();
	}
	...
	public void onInvokeTimeout(MAPDialog mapDialog, Long invokeId) {
		...
		// TODO Auto-generated method stub
		dialogHandler.handleTxn(mapDialog.getLocalDialogId(), 5);
		Metrics.globalRegistry.counter("map_response", "type", "AnyTimeInterrogation", "exception", "TIMEOUT")
				.increment();
	}
