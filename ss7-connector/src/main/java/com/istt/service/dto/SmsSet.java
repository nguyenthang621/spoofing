...
public class SmsSet implements Serializable {
  ...
  public void updateDueDelay(int dueDelay) {
    if (this.dueDelay > dueDelay) this.dueDelay = dueDelay;
  }
  ...
  /**
   * 0-subscriber target not schedulered by deuDate, 1-schedulered by deuDate, 2-delivering in
   * processed
   */
  ...
  /**
   * ErrorCode value will be put here for last attempt (0==success / no attempts yet, !=0 �
   * ErrorCode of the last attempt)
   */
  ...
  public void addSmsSet(SmsSet smsSet) {
    ...
    if (i1 == 0) {
      ...
      //            if (this.segmList.get(i1 - 1).smsList.size() + al.size() >
      // SmsSetCache.SMSSET_MSG_PRO_SEGMENT_LIMIT) {
      //                for (int i2 = 0; i2 < al.size(); i2++) {
      //                    Sms smsx = al.get(i2);
    ...
    //            SmsSetCache.getInstance().registerSmsSetWithBigMessageCount(this.getTargetId(),
    // this);
  }
  ...
  public Sms getSms(long index) {
    ...
    for (int i1 = 0; i1 < segmCnt; i1++) {
      ...
      if (indexCur < segm.cnt) {
        ...
        if (al != null) return al.get((int) indexCur);
        else return null;
      } else {
        indexCur -= segm.cnt;
      }
  ...
  public boolean equals(Object obj) {
    ...
    if (this.destAddrTon != other.destAddrTon) return false;
    if (this.destAddrNpi != other.destAddrNpi) return false;
    if (this.networkId != other.networkId) return false;

    if (destAddr == null) {
      if (other.destAddr != null) return false;
    } else if (!destAddr.equals(other.destAddr)) return false;
  ...
  public class SmsComparator implements Comparator<Sms> {
    ...
    public int compare(Sms a1, Sms a2) {
      if (a1.getMessageId() == a2.getMessageId()) return 0;
      else {
        if (a1.getMessageId() < a2.getMessageId()) return -1;
        else return 1;
      }
    }
  }
