...
public class SCTPConfiguration {
  ...
  public void initialize() throws Exception {
    ...
    log.info("======== SCTPConfiguration ===============");
    sctpManagement = new ManagementImpl(props.getSs7Name());
    sctpManagement.setPersistDir(props.getPersistDir());
    sctpManagement.setSingleThread(false);
    if (props.getSctpWorkerThread()>0) {
    	log.info("set number of worker threads: {}",props.getSctpWorkerThread());
    	sctpManagement.setWorkerThreads(props.getSctpWorkerThread());
    }
            ...
            entry -> {
              ...
              AtomicInteger status =
                  Metrics.globalRegistry.gauge(
                      ...
                      Tags.of(
                          "name", assocName,
                          "host", assoc.getHostAddress() + ":" + assoc.getHostPort(),
                          "peer", assoc.getPeerAddress() + ":" + assoc.getPeerPort()),
